    ...
    <header id="header" class="header d-flex align-items-center fixed-top">
        <div class="container-fluid container-xl d-flex align-items-center justify-content-between header-atas">
            ...
            <a href="/" class="logo d-flex align-items-center tulisan-nusantara" style="text-decoration: none;">
                ...
                <h1 class="tulisan-nusantara2">NusantaraBlog</h1>
            </a>
    ...
    <main id="main">
        <section>
            <div class="container">
                ...
                <div class="row mb-5">
                    <div class="col-12">
                        <a href="/Dashboard" class="btn btn-primary"><i class="fa fa-plus me-1" aria-hidden="true"></i>Tambah Postingan Blog</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-9" data-aos="fade-up">
                        <h3 class="category-title">Category: All Blog</h3>

                        @foreach($AllBlog as $blg)
                        <div class="d-md-flex post-entry-2 half">
                            <a href="/detailbloghome/{{ $blg->slug }}" class="me-4 thumbnail">
                                <img src="{{ asset('storage/' . $blg->blog_image) }}" alt="" class="img-fluid">
                            </a>
                            <div>
                                <div class="post-meta"><span class="date">{{ $blg->slug_category }}</span> <span class="mx-1">&bullet;</span> <span class="published_at">{{ $blg->published_at  }}</span><span class="mx-2">&bullet;</span> <span class="negara">{{ $blg->negara  }}</span></div>
                                <h3><a href="/detailbloghome/{{ $blg->slug }}" class="ubah" style="text-decoration: none;">{{ $blg->title }}</a></h3>
                                <p>{!! Illuminate\Support\Str::limit($blg->body, $limit = 390, $end = 'END') !!}</p>
                                <div class="d-flex align-items-center author">
                                    <div class="photo"><img src="{{ asset('storage/' . $blg->image) }}" alt="" class="img-fluid h-10"></div>
                                    <div class="name">
                                        <h3 class="m-0 p-0 nameuser">{{ $blg->name }}</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
